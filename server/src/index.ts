...
app.post('/generate-diagram', generateDiagramController as any)

// Debug endpoint for Gemini API connectivity/model discovery (MVP-only)
const geminiModelsHandler: AsyncHandler = async (req, res) => {
  try {
    ...
    if (!r.ok)
      return res.status(r.status).json({
        error: 'Gemini list models failed',
        details: text
      })
    ...
    const simplified = models.slice(0, 50).map((m: any) => ({
      name: m?.name,
...
app.get('/gemini/models', geminiModelsHandler as any)
